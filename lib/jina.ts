 ...
 *   4. Search    — web search for ranking monitoring
 */
...
// ---------------------------------------------------------------------------
// Helpers — build passage for content embedding
// ---------------------------------------------------------------------------

export function buildServicePassage(service: {
  name: string;
  description: string;
  industry?: string;
  results?: string[];
  ...
  const parts = [
    ...
    service.description,
    service.industry ? `Setor: ${service.industry}` : '',
    service.results?.length ? `Resultados: ${service.results.join('; ')}` : '',
  ];
